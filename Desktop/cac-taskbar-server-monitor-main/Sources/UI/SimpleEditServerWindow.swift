...
class SimpleEditServerWindow: NSWindowController, NSWindowDelegate {
    ...
    init(window: NSWindow?, serverRepository: ServerRepository, serverToEdit: ServerConfig) {
    ...
    convenience init(serverRepository: ServerRepository, serverToEdit: ServerConfig) {
        let window = NSWindow(
        ...
        self.init(window: window, serverRepository: serverRepository, serverToEdit: serverToEdit)
    }
    ...
    private func populateFields() {
        ...
        passField.stringValue = serverToEdit.password ?? ""
        keyField.stringValue = serverToEdit.keyPath ?? ""
    ...
    @objc private func saveButtonClicked() {
        ...
        // Create updated server config with the same ID and enabled status
        ...
        let validationErrors = serverRepository.validateServer(updatedServer)
        if !validationErrors.isEmpty {
            showAlert("Validation Error", validationErrors.joined(separator: "\n"))
            return
        }
        
        if serverRepository.updateServer(updatedServer) {
            print("✅ Server '\(updatedServer.name)' updated successfully.")
            onServerUpdated?()
            self.close()
        } else {
            print("❌ Failed to update server.")
            showAlert("Error", "Failed to update server in configuration file.")
        }
