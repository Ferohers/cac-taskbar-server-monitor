...
class SimpleAddServerWindow: NSWindowController {
    ...
    @objc private func addButtonClicked() {
        ...
        if serverRepository.addServer(server) {
            print("✅ Server '\(server.name)' saved successfully.")
            onServerAdded?()
            self.close()
        } else {
            print("❌ Failed to save server.")
            showAlert("Error", "Failed to save server to configuration file.")
        }
    }
