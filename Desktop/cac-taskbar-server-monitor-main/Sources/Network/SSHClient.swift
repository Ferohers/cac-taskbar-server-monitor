...
struct NetworkInfo {
    ...
    init(uploadMbps: Double, downloadMbps: Double, downloadStr: String? = nil, uploadStr: String? = nil) {
...
class SSHClient {
    ...
    func connect(to config: ServerConfig) throws {
        ...
        let testCommand = buildSSHCommand(command: "echo 'connected'", config: config)
    ...
    func getCPUUsage() throws -> Double? {
        guard isConnected, let config = serverConfig else {
            throw SSHError.connectionFailed("Not connected")
        }
        
        // Simplified command to avoid complex nested quoting issues, focusing on Linux /proc/stat
        let command = "grep 'cpu ' /proc/stat | awk '{usage=($2+$4)*100/($2+$3+$4+$5)} END {print usage}'"
        let sshCommand = buildSSHCommand(command: command, config: config)
        ...
        if let range = output.range(of: #"\d+\.?\d*"#, options: .regularExpression) {
            let numberString = String(output[range])
    ...
    func getMemoryUsage() throws -> MemoryInfo {
        ...
        guard components.count >= 2,
              let usage = Double(components[0]),
              let total = Double(components[1]) else {
    ...
    private func getContainerStartTimes(_ containers: [DockerContainer], config: ServerConfig) throws -> [DockerContainer] {
        ...
        for container in containers {
            ...
            if container.status.lowercased().contains("up") {
                ...
                let sshCommand = buildSSHCommand(command: inspectCommand, config: config)
                let result = executeCommand(sshCommand)
    ...
    func getNetworkUsage() throws -> NetworkInfo {
        ...
        if result.exitCode != 0 {
            return NetworkInfo(uploadMbps: 0.0, downloadMbps: 0.0)
        }
        ...
        guard components.count >= 2,
              let downloadBytes = Double(components[0]),
              let uploadBytes = Double(components[1]) else {
            return NetworkInfo(uploadMbps: 0.0, downloadMbps: 0.0)
        }
    ...
    func getDiskSpace() throws -> DiskInfo {
        ...
        guard components.count >= 3,
              let availableGB = Double(components[0]),
              let usagePercent = Double(components[1]),
              let totalGB = Double(components[2]) else {
            throw SSHError.invalidResponse
        }
    ...
    private func restartComposeService(project: String, service: String, workingDir: String, config: ServerConfig) throws -> Bool {
        ...
        """
        let sshCommand = buildSSHCommand(command: restartCommand, config: config)
        let result = executeCommand(sshCommand)
    ...
    private func buildSSHCommand(command: String, config: ServerConfig) -> String {
        ...
        if let keyPath = config.keyPath, !keyPath.isEmpty {
            ...
            // Properly escape the key path to handle spaces and special characters
